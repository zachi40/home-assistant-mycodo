...
class MycodoConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            ...
            # Validate API Key
            if not self._errors and not await self._check_api_key(user_input[CONF_IP_ADDRESS], user_input[CONF_API_KEY], protocol):
    ...
    async def _check_api_key(self, ip, api_key, protocol):
        ...
        http_headers = {"accept": "application/vnd.mycodo.v1+json", "X-API-KEY": api_key}
